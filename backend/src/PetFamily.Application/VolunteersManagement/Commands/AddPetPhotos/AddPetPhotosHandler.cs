...
public class AddPetPhotosHandler : ICommandHandler<string, AddPetPhotosCommand>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // upload files
        var uploadResult = await _fileProvider.UploadObjects(command.Files, command.BucketName.ToLower(), cancellationToken);
        if(uploadResult.IsFailure)
        {
            await _messageQueue.WriteAsync(command.Files
                .Select(f => new FileInfo(f.ObjectName.Value, command.BucketName.ToLower())), cancellationToken);
